...
public class TCommunicaIoHandle extends IoHandlerAdapter {
	...
	public void messageReceived(IoSession session, Object message) throws Exception {
		FileContent f = (FileContent) message;
		if (f.getCommand() == Communication.closeCommand) {
			...
			while (entries.hasNext()) {
				...
				IoSession session1 = entry.getValue();
				String ip = ((InetSocketAddress) session1.getRemoteAddress()).getAddress().getHostAddress();
				if (ip.equals(clientIP)) {
					Constant.studentSession.remove(entry.getKey());
